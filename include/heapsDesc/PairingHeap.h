...
private:
	...
	pairing_heap_with_buffer<T, B/sizeof(T), Compare> heap_of_elements; //обычные
	pairing_heap_with_buffer<T, B/sizeof(T), Compare> heap_of_del_elements; //удалённые
	
	void flush_dels(){ //убираем удаленные, пока он сравнимы с top
		if(!heap_of_del_elements.empty() && !heap_of_elements.empty() && heap_of_del_elements.getMin() == heap_of_elements.getMin()){ 
...
public:
	...
	explicit PairingHeap(const Compare &comp_): comp(comp_), heap_of_elements(comp), heap_of_del_elements(comp){
	}
	...
	[[nodiscard]] size_t size() const { 
		return heap_of_elements.size() - heap_of_del_elements.size(); //количество добавленных мину количество удаленных
	}
