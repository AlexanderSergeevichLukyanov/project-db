...
TEST_CASE("funnel-heap-with-compare: 3000 insert and 3000 extract_min") {
    ...
    for (int i = 0; i < 30; ++i) {
		int s= h1.getMin() - 10;
        CHECK_MESSAGE(std::abs(s) - res[i] == 0, std::to_string(std::abs(s))+"!="+std::to_string(res[i]));
		//if(std::abs(s) != res[i]) break;
        h1.extractMin();
    }
}
...
//    CHECK(!ch.empty());
    ...
    heap.insert(15);
//    CHECK(!ch.empty());
    CHECK_MESSAGE(ch.getMin() == 15, std::to_string(ch.getMin()));
...
//    CHECK(!ch.empty());
    ...
    heap.extractMin();
//    CHECK(!ch.empty());
    CHECK_MESSAGE(ch.getMin() == 10, std::to_string(ch.getMin()));
