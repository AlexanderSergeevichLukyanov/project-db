...
TEST_CASE("tower-heap: constructors without Compare") {
    TowerHeap_t<int> h1(0);
    //TowerHeap_t<double> h2(0);
    // only compile
}



TEST_CASE("tower-heap: 30000 add") {
...
TEST_CASE("tower-heap: 300000 add") {
    TowerHeap_t<uint64_t> h1(0);
...
TEST_CASE("tower-heap: 300000 top()") {
    TowerHeap_t<uint64_t> h1(0);
    int min = 300000;
    for (int i = 0; i < 300000; ++i) {
        int x = rand() % 300000;
...
TEST_CASE("tower-heap: 3000 add and 3000 extract_min") {
    TowerHeap_t<uint64_t> h1(0);
    std::vector<int> res;
...
TEST_CASE("tower-heap: 10000 add and 10000 extract_min") {
    TowerHeap_t<uint64_t> h1(0);
    std::vector<int> res;
...
TEST_CASE("tower-heap: random test#1") {
    TowerHeap_t<uint64_t> h1(0);
    CHECK(h1.empty());
    CHECK(h1.size() == 0);
    h1.add(1);
...
TEST_CASE("tower-heap: random test#2") {
    TowerHeap_t<uint64_t> h1(0);
    CHECK(h1.empty());
    CHECK(h1.size() == 0);
    h1.add(1.0);
...
/*
TEST_CASE("tower-heap: default contructor must be implicit") {
    [[maybe_unused]] TowerHeap_t<int> h1(0) = {};
    // only compile
}
...
TEST_CASE("tower-heap: constructors with Compare") {
    TowerHeap_t<int, CloserTo> heap(CloserTo(10));
    // only compile
}



TEST_CASE("tower-heap-with-compare: 30000 add") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    for (int i = 0; i < 30000; ++i) {
        h1.add(rand());
    }
    CHECK(h1.size() == 30000);
...
TEST_CASE("tower-heap-with-compare: 300000 add") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    for (int i = 0; i < 300000; ++i) {
        h1.add(rand());
    }
...
TEST_CASE("tower-heap-with-compare: 3000 add and 3000 extract_min") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    std::vector<int> res;
    for (int i = 0; i < 3000; ++i) {
        int x = rand() % 3000;
...
TEST_CASE("tower-heap-with-compare: 10000 add and 10000 extract_min") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    std::vector<int> res;
    for (int i = 0; i < 10000; ++i) {
        int x = rand() % 10000;
...
TEST_CASE("tower-heap-with-compare: random test") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    CHECK(h1.empty());
    CHECK(h1.size() == 0);
    h1.add(1);
...
TEST_CASE("tower-heap-with-compare: check const, reference methods") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    h1.add(7);
    h1.add(1);
    CHECK_MESSAGE(std::is_reference<decltype(h1.top())>::value,
...
TEST_CASE("tower-heap-with-compare: voids must be voids...") {
    TowerHeap_t<int, CloserTo> h1(0)(CloserTo(10));
    h1.add(7);
    h1.add(1);
    int p[10]{};
...
TEST_CASE(
    "tower-heap-with-compare: constructor from comparator is explicit") {
    CHECK((std::is_constructible_v<TowerHeap_t<int>, std::less<int>>));
    CHECK((!std::is_convertible_v<std::less<int>, TowerHeap_t<int>>));
}

TEST_CASE("tower-heap-with-compare: with std::greater(on max)") {
    TowerHeap_t<int, std::greater<>> heap;
    auto &ch = heap;
...
TEST_CASE("tower-heap: delete"){
	TowerHeap_t<int, CloserTo> h(CloserTo(10));
	for(int i=0; i<10; ++i){
		h.add(i);
	}
	h.del(9);
...
TEST_CASE("tower-heap: decreaseKey"){
	TowerHeap_t<int> h;
	for(int i=0; i<10; ++i){
		h.add(i);
	}
...
*/
void add(std::multiset<int> &pq, TowerHeap_t<uint64_t> &ph){
	int x = rand();
	pq.insert(x);
	ph.add(x);
}

void extract(std::multiset<int> &pq, TowerHeap_t<uint64_t> &ph, int k){
	int x = *(pq.begin());
	int y = ph.top();
	pq.erase(pq.begin());
	ph.extract();
...
/*
void delet(std::multiset<int> &pq, TowerHeap_t<int> &ph, int k){
