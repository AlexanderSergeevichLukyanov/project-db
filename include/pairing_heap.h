...
template<typename T> bool Empty(HeapNode<T> *node) {  
    return (node == NULL); 
} 
  
template<typename T, typename Compare = std::less<T>> HeapNode<T> *Merge(HeapNode<T> *A, HeapNode<T> *B, Compare &comp)  
{ 
...
template<typename T> T top(HeapNode<T> *node) { 
    return node->key;  
} 
  
template<typename T, typename Compare=std::less<T>> HeapNode<T> *Insert(HeapNode<T> *node, T &key, Compare &comp) { 
    return Merge(node, new HeapNode(key, NULL, NULL), comp); 
} 
  
template<typename T, typename Compare=std::less<T>> HeapNode<T> *TwoPassMerge(HeapNode<T> *node, Compare &comp) {  
...
template<typename T, typename Compare=std::less<T>> HeapNode<T> *Delete(HeapNode<T> *node, Compare &comp) { 
