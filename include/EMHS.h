...
void READ(const char *DirectoryName, uint64_t NextWrite, Block_t &Block) {
    std::unique_ptr<FILE, decltype(&fclose)> File(
        fopen(NameMake(DirectoryName, NextWrite).c_str(), "rb"), &fclose);
...
void WRITE(const char *DirectoryName,
           uint64_t NextWrite,
           const Block_t &Block) {
    std::unique_ptr<FILE, decltype(&fclose)> File(
        fopen(NameMake(DirectoryName, NextWrite).c_str(), "wb"), &fclose);
    for (std::size_t Index = 0; Index < Block_t::capacity(); Index++) {
...
private:
    ...
    friend void dp(const char *InputDirectory,
...
template <typename Heap_t, typename Calculator_t>
void dp(const char *InputDirectory,
        const char *OutputDirectory,
        Heap_t Heap,
        Calculator_t Calculator) {
