...
private:
    ...
    public:
        ...
        [[nodiscard]] HelminthElement_t upchuck() {
            if (size() % BlockCapacity() == 0) {
                ...
                for (std::size_t Index = sizeof(HelminthElement_t) / 8;
                     Index < 2 * sizeof(HelminthElement_t) / 8; Index++) {
            ...
            HelminthElement_t *Pointer = &Result;
            auto MovablePointer = reinterpret_cast<uint64_t *>(Pointer);
            for (std::size_t Index = (size() % BlockCapacity() + 1) *
                                     sizeof(HelminthElement_t) / 8;
                 Index <
                 (size() % BlockCapacity() + 2) * sizeof(HelminthElement_t) / 8;
                 Index++) {
                *MovablePointer = Head[Index];
                MovablePointer++;
            }
        ...
        [[nodiscard]] std::size_t nibble(HelminthElement_t Value,
                                         uint64_t NextWriteLocale) {
            HelminthElement_t *Pointer = &Value;
            auto MovablePointer = reinterpret_cast<uint64_t *>(Pointer);
            std::vector<uint64_t> Result;
            for (std::size_t Index = 0; Index < sizeof(HelminthElement_t) / 8;
                 Index++) {
            ...
            for (std::size_t Index = (size() % BlockCapacity() + 1) *
                 ...
                 (size() % BlockCapacity() + 2) * sizeof(HelminthElement_t) / 8;
                 Index++) {
                Head[Index] = Result[Index - (size() % BlockCapacity() + 1) *
                                                 sizeof(HelminthElement_t) / 8];
            }
            return 0;
        }
    ...
    public:
        ...
        Flower_t(uint64_t Address, std::size_t DataElementsCount)
            : DataElementsCount(DataElementsCount) {
            Block_t<> Block;
            READ(Address, Block);
            Level = Block[0];
            ...
            auto MovablePointer = reinterpret_cast<uint64_t *>(Pointer);
            for (std::size_t Index = 3; Index < sizeof(Element_t) / 8 + 3;
                 Index++) {
        ...
        [[nodiscard]] std::size_t add(Element_t Value,
                                      uint64_t NextWriteLocale) {
    ...
    void Unfroze(uint64_t Address, std::vector<Flower_t *> &Input) {
        ...
        while (!TmpHelminth.empty()) {
            Input.push_back(
                new Flower_t{TmpHelminth.upchuck(), DataElementsCount});
        }
    ...
    void Cut(Flower_t *Input,
             Helminth_t<Element_t> *&ResultHelminth,
             std::vector<Flower_t *> &ResultVector) {
    ...
    void Cut(Flower_t *Input, std::vector<Flower_t *> &Result) {
        ...
        for (std::size_t Index = 0; Index < 2; Index++) {
            Result.push_back(
                new Flower_t{*TmpElements.begin(), 0, 0, DataElementsCount});
            for (uint64_t SecondIndex = 0; SecondIndex < DataElementsCount;
                 SecondIndex++) {
                NextWrite +=
                    Result.back()->add(*TmpElements.begin(), NextWrite);
    ...
    void ReCut(const std::vector<Flower_t *> &Input,
               std::vector<Flower_t *> &Result) {
        Result.push_back(new Flower_t{Input[0]->border(), Input[0]->level() + 1,
                                      Froze(Input), DataElementsCount});
    }
    ...
    void Normalize() {
        ...
        for (std::size_t Index = 1; Index < DataFlowers.size(); Index++) {
            CurrentCount++;
            if (DataFlowers[Index]->level() !=
                DataFlowers[Index - 1]->level()) {
                CurrentCount = 1;
            }
            if (CurrentCount == 2 * DataBlocksCount) {
                while (Index + 1 < DataFlowers.size() &&
                       DataFlowers[Index + 1]->level() ==
                           DataFlowers[Index]->level()) {
                    Index++;
                }
                std::vector<Flower_t *> TmpDataFlowers;
                for (std::size_t SecondIndex = 0;
                     SecondIndex <= Index - DataBlocksCount; SecondIndex++) {
                    TmpDataFlowers.push_back(DataFlowers[SecondIndex]);
                }
                std::vector<Flower_t *> ForMergeDataFlowers;
                for (std::size_t SecondIndex = Index - DataBlocksCount + 1;
                ...
                for (std::size_t SecondIndex = Index + 1;
                     SecondIndex < DataFlowers.size(); SecondIndex++) {
                    TmpDataFlowers.push_back(DataFlowers[SecondIndex]);
                }
...
public:
    ...
    FlowerHeap_t(uint64_t DirectoryName, std::size_t DataElementsBytes)
        : NextWrite(DirectoryName),
          DataElementsCount(DataElementsBytes / sizeof(Element_t)),
          DataBlocksCount(DataElementsBytes / B){};
    ...
    void add(Element_t Value) {
        Size++;
        int Number = 0;
        while (Number < DataFlowers.size() &&
               DataFlowers[Number]->border() <= Value) {
            Number++;
        }
        Number--;
        if (Number != -1) {
            ...
            if (DataFlowers[Number]->full()) {
                ...
                for (std::size_t Index = Number + 1; Index < DataFlowers.size();
                     Index++) {
                    TmpDataFlowers.push_back(DataFlowers[Index]);
                }
                DataFlowers = TmpDataFlowers;
        ...
        if (DataElements.size() == 3 * DataElementsCount) {
            std::multiset<Element_t> TmpDataElements;
            for (std::size_t Index = 0; Index < 2 * DataElementsCount;
                 Index++) {
                TmpDataElements.insert(*DataElements.begin());
                DataElements.erase(DataElements.begin());
            }
            DataFlowers.emplace_back();
            for (std::size_t Index = DataFlowers.size() - 1; Index >= 1;
                 Index--) {
